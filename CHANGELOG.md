...
- An issuance policy that requires approval now binds every operator path that issues a certificate: signing a stored request (alone or in bulk) and renewing a certificate are queued for approval like the issue form, and approving them performs the signing or the renewal. Administrators bypass as before; the bulk operations report the requests they queued; a request queued for approval is typed after the operation on the Approvals page
