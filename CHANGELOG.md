...
### Changed
- The Active Directory Connector requires a bind DN and a bind password while it is enabled, in the API as well as in the settings form, and refuses to bind without both wherever it binds: lookups, the scheduled health probe and either connection test. Missing either half makes ldap3 bind anonymously, which exposes the certificate subject UCM derives from the directory to whatever an anonymous reader can see, or fails every lookup silently. Turning a connector off needs no credential, filling one in field by field still works, and a blank password field still means "unchanged". On upgrade, a connector that is enabled and has no password stops answering lookups until credentials are entered in the settings dialog, and an API client must send both on the first save of an enabled connector; Active Directory refuses anonymous binds by default, so in practice such a connector was already answering nothing (#363, by @stefanelul2000).
