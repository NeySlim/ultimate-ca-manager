...
### Fixed
- A delegated OCSP responder configured in UCM now actually signs the responses. UCM issued no certificate carrying `id-pkix-ocsp-nocheck`, which its own responder requires (RFC 6960 §4.2.2.2.1), so a certificate configured for that role was refused at answer time and the CA kept signing with its own key: the responses carried the CA's identity in `responderID`, which for the configured certificate is its authority key identifier, and clients that recompute the responder's key hash rejected the signature. A certificate issued with the OCSP signing extended key usage now carries the extension, on every issuance path, the approval workflow, SCEP templates and both renewals included, so a responder issued before this release gets the extension when it is renewed; the list of eligible responders applies the same rule as the responder, and assigning a certificate that lacks the extension is refused with that reason instead of being accepted and ignored. A revoked responder no longer signs, and revoking it drops every answer it had signed from the cache; assigning or removing a responder drops the cache as well, since a cached answer keeps the previous signer and was served with it until it expired; a record that holds a key but no certificate yet is refused as responder instead of being accepted and ignored; and a signing request that already carries the extension is signed instead of being rejected for a duplicate. Assigning a responder now applies the full rule the responder applies at answer time, validity dates, issuer and signature included, instead of a partial copy that accepted a certificate the responder then refused silently; and a responder taken off a certificate hold drops the answers the CA signed while it was on hold. The responder's private key is now loaded and checked to be the certificate's own before the certificate is offered, assigned or used, since a key stored next to a certificate it does not belong to signed answers no client could verify; the certificate and CA imports refuse a PEM whose private key does not match its certificate, which was the way such a pair could get in. Re-importing a certificate or a CA that already exists, with a new certificate and no key, now drops the stored key when the new certificate is not that key's, and says so, instead of keeping a key that no longer belongs to it; a responder holding an Ed25519 or Ed448 key, which passes every check, now signs its answers, the single-certificate path having handed those keys a digest they refuse, so every answer from such a responder came back as an error. Importing a certificate with an Ed25519 or Ed448 private key, or uploading such a key onto a certificate or a request, no longer fails with "Invalid input": those keys have no traditional OpenSSL form and are stored as PKCS#8, which every reader of the stored key accepts; RSA, EC and DSA keys keep their existing form. A CA whose key lives in an HSM gets the same treatment on re-import: the HSM key's public key is compared with the new certificate, a re-keyed certificate unbinds the HSM key and says so, a certificate that is that key's keeps it, and an HSM that cannot be reached refuses the update rather than keeping or dropping the binding on a guess. A responder holding a DSA key now signs the answer to a request carrying several certificates, that path having refused the key type while the single-certificate one accepted it. A responder certificate whose Key Usage extension omits digitalSignature is refused, at assignment, in the list of eligible certificates and at answer time, since RFC 5280 forbids such a key to sign anything but certificates and CRLs and clients that honour the extension reject the signature. And a stored private key that cannot be read, corrupt or encrypted with a key the instance no longer has, no longer counts as belonging to another certificate: re-importing a certificate or a CA over it is refused with that reason and the key is kept, as for an HSM key that cannot be reached, instead of being dropped on a read error. A refused re-import now leaves the record exactly as it was: the key checks run before the record is modified, since the HSM lookup may commit its cache on the way and used to persist the new certificate with the old binding despite the refusal. And when two CAs share a subject, or two certificates share a subject and an issuer, a re-import now lands on the record holding the certificate's key, the very same certificate first, instead of on whichever record the database returned first, which overwrote a homonym and dropped its key while the real target stayed as it was; a certificate whose key links it to none of several homonyms is refused with that reason, since the record to update cannot be determined. A cross-signed CA, the same key under several issuers, is told apart by the issuer of the renewed certificate, by the issuer's key and not only its name, since two parents may share a name: the authority key identifier when the certificates carry one, else the signature checked against the parents known to UCM; and the refusal, when the records really cannot be told apart, now says so accurately; a CA still waiting for its certificate is recognised by the key of its signing request, or by the public key of its HSM key, and is completed through the same path as the dedicated certificate upload (key match, validity, chain to the issuer, activation) instead of being patched in place or refused as ambiguous. Both import routes now record an imported CA's serial number, keep it and the subject key identifier in step with the certificate on re-import, and apply a revocation the parent still holds for the new serial, which only one of the two routes did. A renewed certificate imported onto a revoked record, certificate or CA, is no longer published as revoked: revocation is per certificate (RFC 5280), the revoked serial stays on the list and the record follows its new certificate; the record's issuing CA and path length are re-resolved from the new certificate, for a certificate imported without a CA by its authority key identifier; a CA holding an outstanding renewal request is completed through the same path as the certificate upload, the request fulfilled and the replaced serial reported; a certificate for a foreign CA that merely shares the subject of a CA still waiting for its certificate creates a new CA instead of being refused, and a pending CA restored without its key takes the key that arrives with its certificate; a CA certificate answering an intermediate request pending in the certificates table is created with that request's key, the request being fulfilled, instead of a keyless CA next to an orphaned request; a CA with an Ed25519 or Ed448 key is refused at import with the reason, since such a CA could sign neither certificates nor CRLs, and an Ed25519 or Ed448 certificate keeps its key type when renewed instead of coming back with an RSA key; and the Smart Import now encrypts private keys at rest like every other import path. An independent review of these changes found more, all fixed: an HSM-backed CA without a delegated responder answered every single-certificate OCSP request with an internal error while multi-certificate requests worked, the key wrapper used there not being one cryptography's builders accept; a delegated responder's answers now carry a nextUpdate no later than the responder's own expiry, and renewing the responder, by hand or by the automatic renewal, drops the answers cached under its previous certificate; a sub-CA whose record no longer points at its real issuer is still found by its certificate and signature, so its OCSP status is good rather than unknown; a certificate looked up by serial is verified against its stored certificate, since the serial column mixes decimal and hexadecimal forms and the number 16 could be answered with the record of the number 10; deleting a certificate that was a CA's delegated responder removes the binding instead of leaving it pointing at a reusable id; when a configured responder is refused at answer time, the responder configuration says why and the signing error names the reason; the HSM public key lookup no longer commits the caller's transaction on its way; the certificate issue form now stores the private key encrypted at rest like every other path; and a request issued through the approval workflow yields the same certificate as the same request issued directly, extra extended key usages included, the approval being linked to the certificate it produced. Certificates, CRLs, OCSP responses and timestamps signed by an HSM-resident key now carry a signature that verifies: the providers hashed with a digest chosen from the key size (SHA-512 for RSA-4096, SHA-384 for P-384) while the signature named SHA-256, so every signature made with such a key, except RSA-2048 and P-256, failed verification; the digest the signature names is now the one the provider signs with, providers that bind the digest to the key (Azure Key Vault for ECDSA, Google Cloud KMS key versions) deciding it. Backups, reviewed the same way: restoring a backup written before the CA revocation fields existed no longer un-revokes a CA the parent still lists as revoked; a CA's offline state, validity dates, origin and CRL, OCSP, AIA and CPS publication settings are restored, where an offline CA came back online with a key it could not use and a restored CA had no dates; a certificate's invalidity date is exported and restored; a revocation record whose CA is unknown to both the database and the backup is skipped and counted instead of aborting the whole restore on PostgreSQL; the older settings backup route and the scheduled backup password apply the same password rule as the system backup, with the reason, and a scheduled backup that fails is recorded in the audit trail; and a restore refused for a wrong password says so instead of "invalid restore parameters" (#347, reported by @j2r2k2)
...
- Restoring a backup that contains a certificate authority still awaiting its externally signed certificate no longer aborts the whole restore. That authority is stored with an empty certificate, which the restore turned into a missing one against a column that forbids it, so nothing after it was restored either; its pending certificate request is now carried in the backup as well, so the authority can still be completed after a restore (follow-up to #343)
- A record whose signing request is stored as an empty value is read the same way everywhere, as the certificate side already is. It was listed among the pending requests while the dashboard, which excluded it, announced a different number just above; the list of pending requests, the counter beside it and the overview shown on the login page now all come from one definition, and that overview counts certificates rather than every record (follow-up to #345)
- A record whose certificate is stored as an empty value is read the same way everywhere. The certificate list treated it as a certificate while the counters, the dashboard and the metrics treated it as a pending request, so it appeared in the list without being counted anywhere; the certificate list and the pending-request list now partition the records exactly, and a certificate authority still awaiting its own certificate is likewise excluded from certificate exports (follow-up to #345)
- The dashboard and the Prometheus metrics no longer count a pending signing request as a certificate. A record holding only a request was included in the certificate totals, so the dashboard showed it twice, once among the certificates and once as a pending request, and the same record inflated the valid count. Both now count the rows that actually hold a certificate, the dashboard status chart adds up to its total again, and the definitions of valid, expiring, expired and revoked live in one place instead of being restated by each view (follow-up to #345)
- The Orphan filter now finds orphans wherever they sit in the list. It selected them from the page just received, so a single orphan further down left the first page empty, and the counter above the list followed the page on screen rather than the whole set. Both are computed by the server now, over every certificate (follow-up to #345)
- A certificate with no validity dates is no longer invisible to every status filter. Its row reads as valid, while the list and the counters both required a future expiry date, so it belonged to no bucket at all and no filter could reach it; it now counts as valid in all three places (follow-up to #345)
...
- An on-demand backup no longer refuses a password with "Invalid backup parameters". The refusal came from an undocumented rule requiring eight distinct characters, which a password like a repeated pattern fails even at sixteen characters, while the dialog's strength meter called it strong. The exact reason is now returned and shown under the field, the dialog applies the same rule as the server before submitting, and the rule accepts six distinct characters once the password reaches sixteen. The dialog also counts characters the way the server does, so a password using emoji is no longer accepted there and refused by the API (#346, reported by @kiar1404-de)
