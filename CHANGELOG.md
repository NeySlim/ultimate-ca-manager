...
- Issuance paths made consistent with each other: restoring a CA key from an exported file refuses a key that is not the CA certificate's own, as the key import does; a request already holding a certificate is not re-signed in place through the AD CS connector, as the local Sign CSR action refuses it; the CA of an issue or Sign CSR request may be named by id or reference without a type error on PostgreSQL; the CA-only key usages of a template (keyCertSign, cRLSign) are refused on non-CA templates and never land on a certificate issued from an older one; the Certificate Transparency policy (embedded SCTs, ct_required) now applies to the issue form, the approval workflow and SCEP as it did to the other paths
...
- Issuance policy rules (allowed key types, maximum number of DNS names, maximum validity) now bind the Sign CSR action and renewal as they bind the issue form and the approval workflow
...
- The certificate presented over mTLS to EST `/simpleenroll` and `/serverkeygen` must be one the EST CA issued and still holds, as for re-enrolment: a revoked certificate could enrol a new one for any subject. The mTLS login refuses a revoked certificate, whatever the proxy checked. Approving a queued SCEP request from the API now issues its certificate, which the client was polling for indefinitely. A SCEP renewal announces itself as a renewal to webhooks, and a certificate presented before its validity start is refused as such
