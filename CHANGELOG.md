...
### Fixed
- A delegated OCSP responder configured in UCM now actually signs the responses. UCM issued no certificate carrying `id-pkix-ocsp-nocheck`, which its own responder requires (RFC 6960 §4.2.2.2.1), so a certificate configured for that role was refused at answer time and the CA kept signing with its own key: the responses carried the CA's identity in `responderID`, which for the configured certificate is its authority key identifier, and clients that recompute the responder's key hash rejected the signature. A certificate issued with the OCSP signing extended key usage now carries the extension, on every issuance path; the list of eligible responders applies the same rule as the responder, and assigning a certificate that lacks the extension is refused with that reason instead of being accepted and ignored (#347, reported by @j2r2k2)
