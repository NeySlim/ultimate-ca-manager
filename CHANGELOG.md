...
### Changed
- The Active Directory Connector requires a bind DN and a bind password, in the API as well as in the settings form. Without either one the connector binds anonymously, which exposes the certificate subject UCM derives from the directory to whatever an anonymous reader can see, or fails every lookup silently; a connector that already has a password is unaffected, since a blank field still means "unchanged" (#363, by @stefanelul2000).
