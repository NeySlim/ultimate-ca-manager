...
- Every issuance path now checks the issuing CA's own validity window and keeps the certificate inside it. Signing a request through EST, WSTEP, ACME or the Sign CSR action did not refuse an expired CA certificate and could issue a certificate outliving it; the user mTLS certificate path ignored a CA taken offline and turned an expired CA into the issuer of a one-day certificate; and no path refused a CA certificate that is not yet valid. The same rule is applied by the issue form, the approval workflow, SCEP and renewal, with a clear refusal instead of a server error
