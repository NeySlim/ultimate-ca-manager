...
### Added
- The Active Directory Connector takes several domain controllers, one per row, instead of a single server. Lookups try them in order and fail over, so one DC being down no longer stops certificate enrollment, and Test Connection binds to every row and reports its result against that row rather than stopping at the first DC that answers. This also makes `Verify SSL Certificate` usable on a multi-DC domain: the single field invited the domain name, which round-robins across every DC while the certificate is still verified against that domain name, and a domain controller certificate issued from the stock template carries only its own host in the SAN, so verification failed whichever DC answered. A connector already configured with one hostname keeps working unchanged.
- A scheduled health check binds to each of the connector's domain controllers on its own schedule, so an unreachable one is known before a client needs it instead of being rediscovered by an enrollment that waits out its connect timeout. An unreachable DC is skipped while it stays down, a DC going down or recovering is logged once rather than on every check, and losing the last reachable one is logged as an error because Kerberos autoenrollment is failing for every client at that point. The settings row carries a Degraded or All DCs down badge, each row in the dialog shows its own last verdict, and the interval is configurable per connector. The stored verdict only ever reorders or thins the list UCM tries: it expires, and it can never rule out every DC, so a probe that is itself broken cannot take enrollment down with it.
