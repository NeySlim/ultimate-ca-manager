...
class TestEd25519Responder:
    """Fifth review of #347: an Ed25519 responder passes every check, so it
    has to sign, which Ed keys do without a digest choice."""

    def test_ed25519_responder_signs_and_verifies(self, app, auth_client, create_ca, create_cert):
        with app.app_context():
            from cryptography.hazmat.primitives.asymmetric import ed25519 as _ed
            ca = create_ca(cn='Ed25519 Responder CA')
            leaf = create_cert(cn='leaf-ed.example.com', ca_id=ca['id'])
            ca_obj = _ca_model(ca); ca_cert = _load_x509(ca_obj)
            from services.hsm.ca_key_loader import get_ca_signing_key
            key = _ed.Ed25519PrivateKey.generate()
            cert = _delegated_certificate(ca_cert, get_ca_signing_key(ca_obj), key)
            row = Certificate(
                refid='ed25519-responder', descr='ed25519', caref=ca_obj.refid,
                crt=base64.b64encode(cert.public_bytes(serialization.Encoding.PEM)).decode(),
                prv=base64.b64encode(key.private_bytes(
                    serialization.Encoding.PEM, serialization.PrivateFormat.PKCS8,
                    serialization.NoEncryption())).decode(),
            ...
            resp = ocsp.load_der_ocsp_response(der)
            assert resp.response_status == ocsp.OCSPResponseStatus.SUCCESSFUL
            assert resp.responder_key_hash == TestResponderIdRfc6960._key_hash(cert)
            key.public_key().verify(resp.signature, resp.tbs_response_bytes)  # raises if wrong

            # and the multi-CertID path, assembled by hand, agrees
