...
class TestSanitizeCrlFilename:
    def test_slugify_and_download_name(self):
        ...
        assert (
            crl_download_filename(ca, delta=True) == "Corp-Root-CA-abcdef12-delta.crl"
        )
...
class TestCertNotBeforeSkew:
    def test_default_skew_is_fifteen_minutes(self):
        ...
        assert nb >= before - timedelta(
            minutes=DEFAULT_CERT_NOT_BEFORE_SKEW_MINUTES + 1
        )
...
class TestTemplateDigestAndUsage:
    def test_issue_honors_template_digest_and_counts_usage(
