...
def create_certificate():
    ...
    try:
        user_role = (
            getattr(g.current_user, "role", None)
            ...
            if policy:
                ...
                if not user_id:
                    return error_response(
                        "Authentication required for approval workflow", 401
                    )
    ...
    try:
        ...
        if normalized_key in EC_CURVES:
            new_key = ec.generate_private_key(
                EC_CURVES[normalized_key], default_backend()
            )
        else:
        ...
        if data.get("organizational_unit"):
            subject_attrs.append(
                x509.NameAttribute(
                    NameOID.ORGANIZATIONAL_UNIT_NAME, data["organizational_unit"]
                )
            )
        if data.get("country"):
            subject_attrs.append(
        ...
        eku_oids = merge_eku_lists(
            profile["eku"], to_object_identifiers(extra_oid_strs)
        )
        ...
        for key in ("san_dns", "san_ip", "san_email"):
            ...
            for val in implicit.get(key) or []:
                ...
                if key == "san_ip" and any(
                    isinstance(s, x509.IPAddress) and str(s.value) == val
                    for s in san_list
                ):
                    continue
        ...
        final_san_uri = [
            s.value for s in san_list if isinstance(s, x509.UniformResourceIdentifier)
        ]
        ...
        if ca.cdp_enabled:
            cdp_urls = [
                url.replace("{ca_refid}", ca.refid) for url in ca.get_cdp_urls()
            ]
            if cdp_urls:
                dist_points = [
                ...
                builder = builder.add_extension(
                    x509.CRLDistributionPoints(dist_points), critical=False
                )
                ...
                x509.CertificatePolicies(
                    [
                        x509.PolicyInformation(
                            policy_identifier=policy_oid, policy_qualifiers=[ca.cps_uri]
                        )
                    ]
                ),
                critical=False,
        ...
        return created_response(
            data=cert_dict, message="Certificate created successfully"
        )
