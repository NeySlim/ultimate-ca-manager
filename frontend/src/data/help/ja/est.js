    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          { label: '提示される証明書', text: 'mTLS 経由の /simpleenroll と /serverkeygen では、EST の CA が署名した証明書はその CA が現在も保持しているものでなければなりません。失効済み、置き換え済み、削除済みの証明書は拒否されます（RFC 7030 §3.3.2）。TLS 層が信頼する別の認証局の証明書は引き続き受け入れられます' },
        ]
      },
      {
        title: 'エンドポイント',
...
- **提示される証明書** — \`/simpleenroll\`と\`/serverkeygen\`では、EST の CA が署名した証明書はその CA が現在も保持しているものでなければなりません。失効済み、置き換え済み、削除済みの証明書は拒否されます（RFC 7030 §3.3.2）。信頼された別の認証局の証明書は引き続き受け入れられます
