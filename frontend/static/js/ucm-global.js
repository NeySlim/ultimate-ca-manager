    ...
    const handler = function(evt) {
        if (evt.detail.pathInfo.requestPath === `/api/ui/certificates/by-refid/${refid}/revoke`) {
            if (evt.detail.xhr.status === 200 || evt.detail.xhr.status === 204) {
                ...
                try {
                    const data = JSON.parse(evt.detail.xhr.responseText);
                    showToast('Error: ' + (data.error || 'Revoke failed'), 'error');
                } catch (err) {
                    showToast('Error: Revoke failed', 'error');
                }
            }
            document.body.removeEventListener('htmx:afterRequest', handler);
        }
...
function approveSCEP(txid) {
    const handler = function(evt) {
        if (evt.detail.pathInfo.requestPath === '/scep/requests/' + txid + '/approve') {
    ...
    document.body.addEventListener('htmx:afterRequest', handler);
    htmx.ajax('POST', '/scep/requests/' + txid + '/approve', {swap: 'none'});
}

function rejectSCEP(txid) {
    const handler = function(evt) {
        if (evt.detail.pathInfo.requestPath === '/scep/requests/' + txid + '/reject') {
            if (evt.detail.xhr.status === 200) {
    ...
    document.body.addEventListener('htmx:afterRequest', handler);
    htmx.ajax('POST', '/scep/requests/' + txid + '/reject', {
        swap: 'none',
        values: { reason: 'Rejected by administrator' }
    });
}
